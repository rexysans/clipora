...
function WatchLater() {
  ...
  const fetchWatchLater = async () => {
    ...
    try {
      const res = await fetch(API_ENDPOINTS.VIDEOS_WATCH_LATER(user.id), {
        credentials: "include",
      });
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-neutral-50 dark:bg-bg px-4">
          <div className="text-center max-w-md">
            ...
            <p className="text-neutral-600 dark:text-textSecondary mb-6">
              Log in to save videos to watch later
            </p>
            <Link
              to="/login"
      ...
      <div className="min-h-screen bg-neutral-50 dark:bg-bg text-neutral-900 dark:text-white">
        <div className="max-w-[90rem] mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          {loading ? (
            ...
            >
              {videos.map((video) => (
                <article key={video.id} className="relative group">
                  {/* Remove button */}
                  <button
                    onClick={(e) => handleRemove(video.id, e)}
                    className="absolute top-2 right-2 z-10 w-8 h-8 bg-black/70 hover:bg-black/90 rounded-full flex items-center justify-center transition-all opacity-0 group-hover:opacity-100"
